...
void write_star_params_a1a2a3asym(VectorXd spec_params, MatrixXd mode_params, MatrixXd noise_params, std::string file_out, std::string identifier){
    ...
    if(outfile.is_open()){
		...
		// ---------------------
		Nchars_params << 5, 20, 20, 20, 10, 16, 16, 16, 10;
		precision_params << 1, 10, 10, 10, 10, 8, 8, 8, 3;
		...
		outfile << "# Input mode parameters. degree / freq / H / W / splitting a1 / a2 / a3 /  b (mag) / alfa (mag) / asymetry / inclination" << std::endl;
...
void write_star_params_Alm(VectorXd spec_params, MatrixXd mode_params, MatrixXd noise_params, std::string file_out, std::string identifier){
	...
	VectorXi Nchars_spec(2), Nchars_params(12), Nchars_noise(3), precision_spec(2), precision_params(12), precision_noise(3);
    ...
    if(outfile.is_open()){
		...
		// ---------------------
		Nchars_params << 5, 20, 20, 20, 10, 20, 16, 16, 16, 14, 16, 10;
		precision_params << 1, 10, 10, 10, 10, 10, 8, 8, 8, 2, 8, 3;
		...
		outfile << "# Input mode parameters. degree / freq / H / W / splitting a1 / eta0 / epsilon_Alm / theta0_Alm / delta_Alm / a3 / asymetry / inclination" << std::endl;
...
void write_star_mode_params_Alm(MatrixXd mode_params, std::string file_out){
	...
	Nchars << 5, 20, 20, 20, 10, 20, 16, 16, 16, 14, 16, 10;
	precision << 1, 10, 10, 10, 10, 10, 8, 8, 8, 2, 8, 3;
