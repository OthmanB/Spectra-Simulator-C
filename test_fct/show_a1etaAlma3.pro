...
pro show_allinfiles, dir_in
	...
	dir_in='/Users/obenomar/tmp/Simulator/Spectra-Simulator-C-1.0.0/test_fct/cfg_plots/build_l_mode_a1etaAlma3/'
	files=file_search(dir_in + '*.in')
	for i=0, n_elements(files)-1 do begin
		print, '[' + strtrim(i,2) + '] Processing: ' + files[i] + '...'
		show_a1etaAlma3,files[i]
	endfor
end
;Visualisation for the cfg of test_lorentzian_a1a2a3
pro show_a1etaAlma3, cfg_file, c_prg_path=c_prg_path
	model_name='build_l_mode_a1etaAlma3'
...
function read_cfg_file, cfg_file
	...
	cfg={range:dblarr(3), el:0, fc_l:0.0, H_l:0.0, gamma_l:0.0, a1:0.0, eta0:0.0, epsilon_nl:0.0, theta:0.0, delta:0.0, a3:0.0, asym:0.0, inc:0.0}
	line=0
	openr, 3, cfg_file
		a=''
		while eof(3) eq 0 do begin
			...
			if a ne 35 then begin ; detect comment by searching '#' as fist caracter... ignore those lines
				...
				if line eq 7 then cfg.eta0=double(a)
				if line eq 8 then cfg.epsilon_nl=double(a)
				if line eq 9 then cfg.theta=double(a)
				if line eq 10 then cfg.delta=double(a)
				if line eq 11 then cfg.a3=double(a)
				if line eq 12 then cfg.asym=double(a)
				if line eq 13 then cfg.inc=double(a)				 
			endif
		endwhile
	close, 3
